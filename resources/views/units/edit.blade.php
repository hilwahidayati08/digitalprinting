...
@section('title', 'Edit Unit - Admin Panel')
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div class="p-6 border-b border-slate-50 bg-slate-50/30">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-base font-bold text-slate-900">Perbarui Data Unit</h2>
                    <p class="text-[12px] text-slate-500">ID Unit: #{{ $unit->unit_id }}</p>
                </div>
            </div>
        </div>

        <form action="{{ route('units.update', $unit->unit_id) }}" method="POST" class="p-6 md:p-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1.5">
                        <label class="text-[13px] font-bold text-slate-700 ml-1 uppercase tracking-wide">Lebar (cm)</label>
                        <input type="number" step="0.1" name="width" value="{{ old('width', $unit->width) }}"
                               class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all outline-none text-sm">
                    </div>
                    <div class="space-y-1.5">
                        <label class="text-[13px] font-bold text-slate-700 ml-1 uppercase tracking-wide">Tinggi (cm)</label>
                        <input type="number" step="0.1" name="height" value="{{ old('height', $unit->height) }}"
                               class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all outline-none text-sm">
                    </div>
                </div>
            </div>
            ...
            <div class="mt-8 pt-6 border-t border-slate-50 flex items-center justify-end gap-3">
                ...
                <button type="submit" class="px-8 py-2.5 bg-[#0f172a] text-white rounded-xl text-sm font-bold shadow-lg hover:bg-slate-800 transition-all flex items-center gap-2">
                    <i class="fas fa-sync-alt opacity-50"></i> Update Unit
                </button>
            </div>
        </form>
    </div>
