...
$portofolios = Portofolios::latest()->limit(4)->get();    $profile = Profiles::first();
    ...
    public function dashboard()
{
    ...
    $revenueThisMonth   = Orders::where('status', 'delivered')
                            ->where('created_at', '>=', $startOfMonth)
                            ->sum('total');
    $revenueLastMonth   = Orders::where('status', 'delivered')
    ...
    $lowStockProducts   = Materials::where('stock', '<=', 10)->count();
    ...
    for ($i = 5; $i >= 0; $i--) {
        ...
        $revenueChart[] = [
            ...
            'revenue' => Orders::where('status', 'delivered')
    ...
    return view('dashboard', compact(
        ...
        'totalSaldoIn', 'totalSaldoOut',
        'totalCategories', 'totalServices', 'totalPortofolios'
    ));
